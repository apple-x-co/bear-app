...
class CaptchaModule extends AbstractModule
{
    ...
    {
        $this->bind()->annotatedWith('g_recaptcha_site_key')->toInstance((string) getenv('G_RECAPTCHA_SITE_KEY'));
        $this->bind()->annotatedWith('g_recaptcha_secret_key')->toInstance((string) getenv('G_RECAPTCHA_SECRET_KEY'));
