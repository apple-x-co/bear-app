...
### SHOW WARNINGS
[{"Level":"Note","Code":1003,"Message":"update `sql_quality_db`.`verification_codes` set `sql_quality_db`.`verification_codes`.`verified_date` = '2024-01-01 12:00:00',`sql_quality_db`.`verification_codes`.`updated_date` = now() where (`sql_quality_db`.`verification_codes`.`id` = 1)"}]
