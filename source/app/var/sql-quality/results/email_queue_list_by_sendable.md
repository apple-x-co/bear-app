...
### EXPLAIN ANALYZE
-> Sort: (case email_queues.priority when 'high' then 0 when 'normal' then 1 else 2 end), email_queues.schedule_date  (cost=0.255 rows=1) (actual time=0.00525..0.00525 rows=0 loops=1)
    -> Filter: ((email_queues.`active` = 1) and (email_queues.schedule_date <= TIMESTAMP'2024-01-01 12:00:00') and (email_queues.max_attempts > email_queues.attempts))  (cost=0.255 rows=1) (actual time=0.00371..0.00371 rows=0 loops=1)
        -> Index lookup on email_queues using idx_email_queues_1 (sent_date=NULL), with index condition: (email_queues.sent_date is null)  (cost=0.255 rows=1) (actual time=0.00329..0.00329 rows=0 loops=1)
