...
### SHOW WARNINGS
[{"Level":"Note","Code":1276,"Message":"Field or reference 'sql_quality_db.email_queue_recipients.email_queue_id' of SELECT #2 was resolved in SELECT #1"},{"Level":"Note","Code":1003,"Message":"delete from `sql_quality_db`.`email_queue_recipients` where ((`sql_quality_db`.`email_queue_recipients`.`email_queue_id` = `sql_quality_db`.`email_queues`.`id`) and (`sql_quality_db`.`email_queues`.`sent_date` <= TIMESTAMP'2024-01-01 12:00:00'))"}]
