...
### SHOW WARNINGS
[{"Level":"Note","Code":1003,"Message":"update `sql_quality_db`.`email_queues` set `sql_quality_db`.`email_queues`.`sent_date` = '2024-01-01 12:00:00',`sql_quality_db`.`email_queues`.`attempts` = 10 where (`sql_quality_db`.`email_queues`.`id` = 1)"}]
